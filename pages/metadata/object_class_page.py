"""
元数据管理 - 对象类 页面 POM
路径: pages/metadata/object_class_page.py
对应 DolphinScheduler 路由: /metadata/object-class
从首页 → 数据平台 → 元数据管理 → 对象类 进入。

页面结构:
- 筛选区: 对象类编码/对象类名称/是否启用/父类编码/初始显示
- 表格列: 序号/对象类编码/对象类名称/英文标识/是否启用/初始显示/父类编码/备注/操作
- 操作列每行4个图标按钮: 查看(info)/编辑(primary)/启用-禁用切换(warning)/删除(error)
- 顶部按钮: 查询/重置/新增/批量删除/批量启用/批量禁用
- 新增弹窗: 对象类编码*/对象类名称*/英文标识*/是否启用/初始显示/父类编码/备注
"""
...
class ObjectClassPage(BasePage):
    """元数据管理 - 对象类 页面。"""

    path = "/metadata/object-class"

    # ---- 菜单导航 ----
    SEL_MENU_METADATA = ".n-menu-item-content:has-text('元数据管理')"
    SEL_MENU_OBJECT_CLASS = ".n-menu-item-content:has-text('对象类')"

    # ---- 筛选区 form-item ----
    SEL_FILTER_FORM = ".n-card .n-form"
    SEL_F_CODE_INPUT = ".n-form-item:has-text('对象类编码') input"
    SEL_F_NAME_INPUT = ".n-form-item:has-text('对象类名称') input"
    SEL_F_ENABLE_SELECT = ".n-form-item:has-text('是否启用') .n-base-selection"
    SEL_F_PARENT_SELECT = ".n-form-item:has-text('父类编码') .n-base-selection"
    SEL_F_INIT_SHOW_SELECT = ".n-form-item:has-text('初始显示') .n-base-selection"
    ...
    SEL_TABLE_CHECKBOX = ".n-data-table-check-box"
    SEL_TABLE_CHECKBOX_ALL = ".n-data-table-th .n-checkbox"

    # ---- 操作列按钮（按颜色类型）----
    # 实际结构（探查确认）：info=查看, primary=编辑, success=启用/禁用切换, error=删除
    SEL_OP_VIEW = ".n-button--info-type"
    SEL_OP_EDIT = ".n-button--primary-type"
    SEL_OP_TOGGLE = ".n-button--success-type"
    ...
    SEL_M_CODE_INPUT = ".n-modal .n-form-item:has-text('对象类编码') input"
    SEL_M_NAME_INPUT = ".n-modal .n-form-item:has-text('对象类名称') input"
    SEL_M_EN_NAME_INPUT = ".n-modal .n-form-item:has-text('英文标识') input"
    SEL_M_PARENT_SELECT = ".n-modal .n-form-item:has-text('父类编码') .n-base-selection"
    ...
    SEL_M_INIT_SHOW_SWITCH = ".n-modal .n-form-item:has-text('初始显示') .n-switch"
    SEL_M_CANCEL_BTN = ".n-modal button:has-text('取消')"
    SEL_M_SAVE_BTN = ".n-modal button:has-text('保存')"

    # ---- 详情弹窗（查看）----
    SEL_DETAIL_MODAL = ".n-modal, .n-drawer"
    SEL_DETAIL_CONTENT = ".n-modal .n-descriptions, .n-modal .n-form, .n-drawer .n-descriptions"

    # ---- 空数据提示 ----
    SEL_EMPTY_TIP = ".n-data-table-empty .n-data-empty, .n-data-empty"

    # ---- 确认弹窗（删除/批量操作/启用禁用切换）----
    # 覆盖三种弹窗：n-dialog（常规对话框）、n-modal（模态框）、n-popconfirm（气泡确认框）
    SEL_CONFIRM_DIALOG = (".n-dialog, .n-popconfirm, "
                          ".n-modal:has-text('确定'), .n-modal:has-text('确认')")
    ...
    # ---- 导航 ----
    @allure.step("从数据平台首页进入对象类页面")
    def goto_object_class(self):
        """点击 元数据管理 父菜单 → 对象类 子菜单。

        兼容 SPA 客户端路由延迟：先等 URL 稳定（/monitor/home 或 /ui/ 加载完成），
        再等菜单容器可见，最后点击。带重试机制。
        """
        ...
        # 3. 对象类 子菜单
        try:
            self.page.locator(self.SEL_MENU_OBJECT_CLASS).first.wait_for(
                state="visible", timeout=8000
            )
        except Exception:
            pass
        self.page.locator(self.SEL_MENU_OBJECT_CLASS).first.click()
        self.page.wait_for_timeout(1500)
        try:
            self.page.wait_for_load_state("networkidle", timeout=15000)
        except Exception:
    ...
    def get_column_values(self, header_name: str) -> list[str]:
        ...
        使用 th[data-col-key] 与 td[data-col-key] 精确匹配，
        避免 checkbox 列导致索引偏移。
        """
        js = """(headerName) => {
    ...
    def select_filter(self, label: str, option: str):
        ...
        # 通用方法：找 form-item，点其内的 n-base-selection
        form_item = self.page.locator(f".n-form-item:has-text('{label}')").first
        form_item.locator(".n-base-selection").first.click()
        self.page.wait_for_timeout(300)
        # 选下拉项
        dropdown = self.page.locator(".n-base-select-option, .n-select-option, .n-base-select-menu")
    ...
    def click_row_toggle(self, row_index: int = 0):
        ...
        if rows.count() > row_index:
            ...
            # 优先 success-type，fallback warning-type（按钮类型可能随状态切换）
            toggle = row.locator(self.SEL_OP_TOGGLE).first
            try:
                toggle.wait_for(state="visible", timeout=3000)
            except Exception:
                toggle = row.locator(".n-button--warning-type").first
            toggle.click()
            self.page.wait_for_timeout(500)
        return self
    ...
    def fill_form(self, code: str = "", name: str = "", en_name: str = "",
                  parent: str = "", remark: str = ""):
        if code:
            self.page.locator(self.SEL_M_CODE_INPUT).first.fill(code)
        if name:
            self.page.locator(self.SEL_M_NAME_INPUT).first.fill(name)
        if en_name:
            self.page.locator(self.SEL_M_EN_NAME_INPUT).first.fill(en_name)
        if parent:
            # 下拉选择父类
            self.page.locator(self.SEL_M_PARENT_SELECT).first.click()
            self.page.wait_for_timeout(300)
            opt = self.page.locator(
                ".n-base-select-option, .n-select-option, .n-base-select-menu"
            ).filter(has_text=parent).first
            try:
    ...
    def wait_for_success_message(self, timeout_ms: int = 3000) -> bool:
        try:
            self.page.locator(self.SEL_MESSAGE_SUCCESS).first.wait_for(state="visible", timeout=timeout_ms)
            return True
        except Exception:
            return False
    ...
    def wait_for_error_message(self, timeout_ms: int = 3000) -> bool:
        try:
            self.page.locator(self.SEL_MESSAGE_ERROR).first.wait_for(state="visible", timeout=timeout_ms)
            return True
        except Exception:
            return False
    ...
    @allure.step("切换弹窗内'初始显示'开关")
    def toggle_modal_init_show(self):
        try:
            sw = self.page.locator(self.SEL_M_INIT_SHOW_SWITCH).first
            sw.click()
            self.page.wait_for_timeout(200)
        except Exception:
            pass
    ...
    def get_modal_form_values(self) -> dict:
        ...
        try:
            ...
            vals["name"] = ""
        try:
            vals["en_name"] = self.page.locator(self.SEL_M_EN_NAME_INPUT).first.input_value()
        except Exception:
            vals["en_name"] = ""
        try:
    ...
    def clear_modal_field(self, field: str):
        """清空弹窗中的某个字段。field: code/name/en_name/remark。"""
        sel_map = {
            "code": self.SEL_M_CODE_INPUT,
            "name": self.SEL_M_NAME_INPUT,
            "en_name": self.SEL_M_EN_NAME_INPUT,
    ...
    def close_detail(self):
        try:
            ...
            for sel in [".n-modal button:has-text('取消')",
                        ...
                        ".n-modal button:has-text('确定')",
                        ".n-modal .n-card-header__close, .n-modal .n-icon"]:
    ...
    def change_page_size(self, size: int):
        """点击分页器每页条数选择器，选择指定条数。"""
        try:
            selector = self.page.locator(".n-pagination .n-pagination-size, "
                                         ".n-pagination [class*='size']").first
            selector.click()
            self.page.wait_for_timeout(300)
    ...
    def find_row_by_column(self, header_name: str, value: str) -> int:
        """在指定列中查找包含 value 的行，返回行索引（0-based），未找到返回 -1。"""
