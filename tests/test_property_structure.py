"""
元数据管理 - 属性结构 模块测试
路径: tests/test_property_structure.py
...
- 操作列: 查看(info) / 编辑(primary) / 删除(error)（3个按钮，无启用禁用切换）
- 新增弹窗: 结构编码*(2位数字) / 结构名称*(85字符) / 事件(85字符) / 备注(85字符)
- 顶部: 新增 / 批量删除（无批量启用/禁用）
"""
import time
import pytest
...
from pages.metadata.property_structure_page import PropertyStructurePage


@allure.feature("元数据管理-属性结构")
@pytest.mark.metadata
class TestPropertyStructure:
    """属性结构页面 CRUD + 搜索 + 批量 + 分页 完整测试。"""
    ...
    def test_list_visible(self, page):
        ...
        with allure.step("步骤2: 进入属性结构页面"):
            op = PropertyStructurePage(data_page).goto_property_structure()

        with allure.step("步骤3: 验证表格可见"):
            assert op.is_table_visible(), "属性结构表格不可见"
        ...
        with allure.step("步骤6: 验证表格行数 ≥ 1"):
            ...
            assert row_count >= 1, f"属性结构列表为空，实际行数: {row_count}"

        with allure.step("步骤7: 验证表格列头完整"):
            headers = op.get_headers()
            expected = ["序号", "结构编码", "结构名称", "事件", "备注", "操作"]
            for col in expected:
        ...
        op.screenshot("property_structure_list")
        data_page.close()
    ...
    def test_search_by_code(self, page):
        """验证按结构编码搜索。"""
        with allure.step("步骤1: 进入属性结构页面"):
            home = HomePage(page)
            data_page = home.click_data_platform()
            op = PropertyStructurePage(data_page).goto_property_structure()

        with allure.step("步骤2: 记录第一行的结构编码"):
            codes = op.get_column_values("结构编码")
        ...
        with allure.step("步骤4: 验证结果包含搜索的编码"):
            result_codes = op.get_column_values("结构编码")
    ...
    def test_search_by_name(self, page):
        ...
        with allure.step("步骤2: 记录第一行的结构名称"):
            names = op.get_column_values("结构名称")
        ...
        with allure.step("步骤4: 验证搜索结果包含搜索名称"):
            result_names = op.get_column_values("结构名称")
    ...
    def test_reset_search(self, page):
        ...
        with allure.step("步骤4: 验证筛选输入框已清空"):
            code_value = data_page.locator(
                PropertyStructurePage.SEL_F_CODE_INPUT).first.input_value()
            assert code_value == "", "重置后筛选框未清空"

        data_page.close()

    @allure.story("新增属性结构 - 正常路径")
    @allure.severity(allure.severity_level.BLOCKER)
    @pytest.mark.P0
    def test_create_success(self, page):
        """验证新增属性结构完整流程。结构编码为2位数字。"""
        with allure.step("步骤1: 进入属性结构页面"):
            home = HomePage(page)
            data_page = home.click_data_platform()
            op = PropertyStructurePage(data_page).goto_property_structure()
        ...
        with allure.step("步骤4: 填写表单（2位数字编码）"):
            ts = int(time.time()) % 1000000
            code = f"{(ts % 90) + 10:02d}"  # 2位数字 10-99
            name = f"AutoTest_PropStruct_{ts}"
    ...
    def test_view_detail_content(self, page):
        ...
        with allure.step("步骤2: 记录第一行结构编码与名称"):
            code = op.get_cell_value(0, "结构编码")
            name = op.get_cell_value(0, "结构名称")
    ...
    def test_pagination_navigation(self, page):
        ...
        with allure.step("步骤2: 检查是否有下一页"):
            try:
                next_btn = data_page.locator(PropertyStructurePage.SEL_PAG_NEXT).first
        ...
        with allure.step("步骤3: 点击下一页"):
            try:
                data_page.locator(PropertyStructurePage.SEL_PAG_NEXT).first.click()
                data_page.wait_for_timeout(1000)
                try:
        ...
        with allure.step("步骤4: 点上一页"):
            try:
                data_page.locator(PropertyStructurePage.SEL_PAG_PREV).first.click()
                data_page.wait_for_timeout(1000)
    ...
    def test_pagination_jump(self, page):
        ...
        with allure.step("步骤2: 记录第一行数据"):
            first_row_before = op.get_cell_value(0, "结构编码")
        ...
        with allure.step("步骤4: 验证数据已切换"):
            first_row_after = op.get_cell_value(0, "结构编码")
    ...
    @allure.story("新增 - 编码非2位数字")
    @allure.severity(allure.severity_level.NORMAL)
    @pytest.mark.P2
    def test_create_invalid_code_format(self, page):
        """异常：结构编码非2位数字（输入字母），应阻止保存或提示错误。"""
        with allure.step("步骤1: 进入属性结构页面"):
            home = HomePage(page)
            data_page = home.click_data_platform()
            op = PropertyStructurePage(data_page).goto_property_structure()
        ...
        with allure.step("步骤3: 填写非法编码（字母，maxlength=2 会截断）"):
            op.fill_form(code="ab", name="AutoTest_InvalidCode", event="invalid")
    ...
    def test_create_duplicate_code(self, page):
        ...
        with allure.step("步骤4: 填入已存在的编码"):
            ts = int(time.time()) % 1000000
            op.fill_form(
                code=existing_code[:2],  # maxlength=2，取前2位
    ...
    def test_create_remark_over_limit(self, page):
        ...
        with allure.step("步骤4: 验证输入框未崩溃且行为合理"):
            actual_remark = data_page.locator(
                PropertyStructurePage.SEL_M_REMARK_TEXTAREA).first.input_value()
